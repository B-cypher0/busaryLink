...
const ADMIN_NAV = [
  ...
  { to:'/funder',    icon:'◆', label:'Funder Portal',   adminOnly:true },
]
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      ...
      <div style={{ padding:'24px 20px 12px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div>
          ...
          <div style={{ fontSize:11, color: admin ? 'var(--purple)' : 'var(--muted)', marginTop:3, letterSpacing:'0.5px' }}>
      ...
      <nav style={{ flex:1, padding:'8px 12px', overflowY:'auto' }}>
        {admin && <div style={{ fontSize:10, color:'var(--muted)', padding:'6px 12px 4px', letterSpacing:'1px', textTransform:'uppercase' }}>Student</div>}
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh' }}>
      ...
      <div className="mobile-header" style={{
        ...
        <div style={{ fontFamily:'Space Grotesk', fontSize:18, fontWeight:700, color:'var(--gold)' }}>
          Bursary<span style={{ color:'var(--text)' }}>Link</span>
        </div>
      ...
      {/* Mobile overlay */}
      {menuOpen && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:199 }} onClick={() => setMenuOpen(false)} />
      )}

      {/* Sidebar */}
      <aside className={`sidebar${menuOpen ? ' open' : ''}`} style={{
        width:'var(--sidebar-w)', background:'var(--dark-2)', borderRight:'1px solid var(--border)',
      ...
      {/* Main */}
      <main className="main-content" style={{ flex:1, overflow:'auto', maxWidth:'calc(100vw - var(--sidebar-w))', paddingTop:0 }}>
        {/* Mobile top padding so content isn't under the header */}
